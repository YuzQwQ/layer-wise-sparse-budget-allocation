"""Summarize R23 H2O-style token-level KV retention diagnostic on C4."""
...
LOCAL_RESULTS_DIR = Path("results") / "round23-phase23-h2o-c4-kv-retention-diagnostic"
...
def collect(input_dir: Path) -> list[dict[str, Any]]:
    ...
    for method in METHOD_ORDER:
        run_dir = input_dir / "formal" / f"r23_c4_{method}"
        ...
        rows.append(
            {
                ...
                "val_actual_tokens": (((metrics or {}).get("val_cache") or {}).get("actual_tokens")),
                "test_actual_tokens": (((metrics or {}).get("test_cache") or {}).get("actual_tokens")),
            }
        )
...
def decision(rows: list[dict[str, Any]]) -> dict[str, str]:
    if len(rows) < 3:
        return {"status": "incomplete", "interpretation": "R23 C4 H2O-style diagnostic is incomplete."}
    best = min(rows, key=lambda row: row["val_loss"] if row["val_loss"] is not None else float("inf"))
    if best["method"] == "mono_inc":
        return {
            "status": "limited_cross_operator_support_on_c4",
            "interpretation": "mono_inc is best in this C4 H2O-style diagnostic; write as limited cross-operator support on C4 only.",
        }
    return {
        "status": "operator_sensitivity_evidence_on_c4",
        "interpretation": f"{best['method']} is best in this C4 H2O-style diagnostic; write as operator sensitivity evidence on C4.",
    }
...
def plot_delta(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fig.suptitle("C4 H2O-style KV retention diagnostic: improvement over uniform")
    fig.tight_layout()
    fig.savefig(out_dir / "h2o_c4_delta_vs_uniform.png")
    plt.close(fig)
...
def plot_budgets(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "h2o_c4_retention_budget_by_layer.png")
    plt.close(fig)


def write_markdown(rows: list[dict[str, Any]], decision_info: dict[str, str], out_path: Path) -> None:
    lines = [
        "# R23 H2O-style Token-level KV Retention Diagnostic on C4",
        "",
    ...
    lines.extend(
        [
            ...
            "- Formal C4 validation/test token windows are required to contain exactly 393,216 tokens each.",
        ]
    )
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (out_dir / "round23_h2o_c4_kv_retention_summary.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    write_markdown(rows, decision_info, out_dir / "round23_h2o_c4_kv_retention_summary.md")
    if rows:
        plot_delta(rows, fig_dir)
        plot_budgets(rows, fig_dir)
    print(f"Wrote R23 summary to {out_dir}")
