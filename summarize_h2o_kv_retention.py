"""Summarize R22 H2O-style token-level KV retention diagnostics."""
...
LOCAL_RESULTS_DIR = Path("results") / "round22-phase22-h2o-kv-retention-diagnostic"
...
def collect(input_dir: Path) -> list[dict[str, Any]]:
    ...
    for method in METHOD_ORDER:
        run_dir = input_dir / "formal" / f"r22_opt350m_{method}"
...
def decision(rows: list[dict[str, Any]]) -> dict[str, str]:
    if len(rows) < 3:
        return {
            "status": "incomplete",
            "interpretation": "R22 H2O-style token-level KV retention diagnostic is incomplete.",
        }
    best = min(rows, key=lambda row: (row["val_loss"] if row["val_loss"] is not None else float("inf")))
    if best["method"] == "mono_inc":
        return {
            "status": "limited_cross_operator_support",
            "interpretation": "mono_inc is best in this H2O-style diagnostic; write as limited cross-operator support only.",
        }
    return {
        "status": "operator_sensitivity_evidence",
        "interpretation": f"{best['method']} is best in this H2O-style diagnostic; write as operator sensitivity evidence.",
    }
...
def plot_delta(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fig.suptitle("H2O-style KV retention diagnostic: improvement over uniform")
    fig.tight_layout()
    fig.savefig(out_dir / "h2o_delta_vs_uniform.png")
    plt.close(fig)
...
def plot_budgets(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "h2o_retention_budget_by_layer.png")
    plt.close(fig)
...
def write_markdown(rows: list[dict[str, Any]], decision_info: dict[str, str], out_path: Path) -> None:
    ...
    for row in rows:
        lines.append(
            "| {sparse_platform} | {sparse_unit} | {model} | {dataset} | {method} | {val} | {ppl} | {dval} | {dppl} | {recent} | {hh} | {total} | {tok} |".format(
                sparse_platform=row["sparse_platform"],
                sparse_unit=row["sparse_unit"],
                model=row["model"],
                dataset=row["dataset"],
                method=row["method"],
...
def main() -> None:
    ...
    (out_dir / "round22_h2o_kv_retention_summary.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    write_markdown(rows, decision_info, out_dir / "round22_h2o_kv_retention_summary.md")
    if rows:
        plot_delta(rows, fig_dir)
        plot_budgets(rows, fig_dir)
    print(f"Wrote R22 summary to {out_dir}")
