"""Summarize R27 sliding-window multi-window robustness diagnostics."""
...
LOCAL_RESULTS_DIR = Path("results") / "round27-phase27-sliding-window-robustness-diagnostic"
        ...
        aggregates.append(
            {
                ...
                "avg_window_size": mean(vals("avg_window_size")) if vals("avg_window_size") else None,
            }
        )
    return aggregates
...
def decisions(aggregates: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for dataset in DATASETS:
        ...
        if best["method"] == "mono_inc":
            result[dataset] = "limited_window_level_support_with_window_robustness"
        elif best["method"] == "uniform":
            result[dataset] = "uniform_strong_default_for_local_window_attention"
        else:
            result[dataset] = "preferred_direction_differs_under_window_level_attention"
    return result
...
def plot_delta(aggregates: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    labels = ["C4", "OpenWebText"]
    x = range(len(labels))
    width = 0.24
    offsets = {"uniform": -width, "mono_inc": 0.0, "mono_dec": width}
    for ax, key, err_key, title, ylabel in [
        (axes[0], "delta_val_mean", "delta_val_std", "Validation loss improvement", "Delta Val = uniform - method"),
        (axes[1], "delta_ppl_mean", "delta_ppl_std", "Test PPL improvement", "Delta PPL = uniform - method"),
    ]:
        for method in METHOD_ORDER:
            values = []
            errors = []
    ...
    fig.suptitle("Sliding-window multi-window robustness: improvement over uniform")
    fig.tight_layout()
    fig.savefig(out_dir / "sliding_window_robustness_delta_mean_std.png")
    plt.close(fig)
...
def write_markdown(rows: list[dict[str, Any]], aggregates: list[dict[str, Any]], decision_info: dict[str, str], out_path: Path) -> None:
    ...
    for row in aggregates:
        lines.append(
            "| {dataset} | {method} | {n} | {val}+/-{val_std} | {ppl}+/-{ppl_std} | {dval}+/-{dval_std} | {dppl}+/-{dppl_std} | {vw}/{n} | {pw}/{n} | {avg} | {tok} |".format(
                dataset=row["dataset"],
                method=row["method"],
                n=row["num_windows"],
    ...
    for row in rows:
        lines.append(
            "| {dataset} | {window} | {vskip} | {tskip} | {method} | {val} | {ppl} | {late} | {dval} | {dppl} | {vtok}/{ttok} |".format(
                dataset=row["dataset"],
...
def main() -> None:
    ...
    (args.output_dir / "round27_sliding_window_robustness_summary.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    write_markdown(rows, aggregates, decision_info, args.output_dir / "round27_sliding_window_robustness_summary.md")
    if aggregates:
        plot_delta(aggregates, fig_dir)
    print(f"Wrote R27 sliding-window robustness summary to {args.output_dir}")
